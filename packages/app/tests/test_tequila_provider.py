...
def test_direct_pass_precedes_stopover_pass(monkeypatch) -> None:
    ...
    monkeypatch.setattr("src.services.providers.tequila.httpx2.Client.get", fake_get)
...
def test_pacing_kicks_in_only_at_the_quota(monkeypatch) -> None:
    from src.services.providers.tequila import RATE_LIMIT_PER_MINUTE

    provider = TequilaProvider("https://t", "key")
    slept: list[float] = []
    clock = {"now": 1000.0}
    monkeypatch.setattr(
        "src.services.providers.tequila.time.monotonic", lambda: clock["now"]
    )
    monkeypatch.setattr(
        "src.services.providers.tequila.time.sleep",
