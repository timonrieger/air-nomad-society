...
from src.services.selection import LOW_TIERS, low_badge, price_low_since
...
# Every wall card must earn at least the lowest badge tier: the wall sells
# subscriptions, and a card without a proven claim sells nothing.
WALL_MIN_LOW_WEEKS = LOW_TIERS[-1][0]
...
def read_deals(session: SessionDep, response: Response) -> list[WallDeal]:
    ...
    observations = route_observations(
        {
            (row.origin_iata or row.departure_iata, row.arrival_iata)
            for row in unique.values()
        },
        before=datetime.now(),
    )
    qualified: list[tuple[SentDeal, LowClaim]] = []
    for row in unique.values():
        route = (row.origin_iata or row.departure_iata, row.arrival_iata)
        claim = price_low_since(
            observations.get(route, []), row.price_eur, before=row.sent_at
        )
        if claim is not None and claim.weeks >= WALL_MIN_LOW_WEEKS:
            qualified.append((row, claim))
    # The cheapest, most comfortable dozen make the wall; within it the
    # longest-standing lows lead.
    best = sorted(qualified, key=lambda pair: _eur_quality(pair[0]))
    best = sorted(best[:WALL_DEAL_COUNT], key=lambda pair: -pair[1].weeks)
    images = load().images
    return [
        WallDeal(
