...
def subscribe():
    ...
    if (token or id) and request.method == "GET":
        ...
        if not member:
            flash(
                "No member found. Please subscribe to become a member.",
                category="error",
            )
            ...
            form = AirNomadSocietyForm(
                ...
                favorite_countries=[
                    country.strip() for country in member.travel_countries.split(",")
                ],
                excluded_countries=[
                    country.strip()
                    for country in (
                        member.excluded_countries.split(",")
                        if member.excluded_countries
                        else []
                    )
                ],
            )
            ...
            db.session.commit()
            flash(
                f"You have successfully unsubscribed with {member.email}.",
                category="success",
            )
            flash(
                "Unsubscribed by mistake? To resubscribe, simply submit the form again."
            )
            ...
            flash(
                "Your profile is ready for updates. Please make any changes as needed.",
                category="success",
            )
            return render_template(
                "subscribe.html", form=form, show_form=True, update=True
            )
    ...
    if form.validate_on_submit() and request.method == "POST":
        already_member = AirNomads.query.where(
            AirNomads.email == form.email.data
        ).scalar()
        favorite_countries = ",".join(
            [country for country in form.favorite_countries.data]
        )
        excluded_countries = (
            ",".join([country for country in form.excluded_countries.data])
            if form.excluded_countries.data
            else None
        )
        if already_member:
        ...
        if not already_member:
            ...
            db.session.commit()
            flash(
                f"Successfully subscribed with {new_member.email}.", category="success"
            )
