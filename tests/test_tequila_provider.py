...
from ans.models import SearchQuery
...
def test_direct_flight_maps_fields(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "ans.providers.tequila.requests.get",
...
def test_stopover_escalation_and_via_city(monkeypatch) -> None:
    ...
    monkeypatch.setattr("ans.providers.tequila.requests.get", fake_get)
...
def test_rate_limited_then_empty_returns_none(monkeypatch) -> None:
    monkeypatch.setattr("ans.providers.tequila.time.sleep", lambda s: None)
    monkeypatch.setattr(
        "ans.providers.tequila.requests.get",
